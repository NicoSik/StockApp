...
/**
 * Coerces to a number, but keeps absent values absent.
 *
 * <p>`Number(null)` is 0, and so is `Number('')`. Coercing first and testing
 * afterwards therefore turns "we do not know" into a confident "0.00%" - a
 * practice account's weight, a holding with no cost basis, an unpriced row.
 * Every formatter below routes through this so a missing value reaches
 * {@link isNum} as NaN and comes back out as a dash.
 */
const toNum = (value) =>
